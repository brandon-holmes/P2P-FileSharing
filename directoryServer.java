	            	...
	            	try
	            	{
		            	...
		            	if (request.equals("2"))//Query Hash
		            	{
		    				...
		    				if(ipLocationStr != null){		    					
			    				...
			    				udpSocket.send(location);
		    				}
		    				else {
		    					String error = "file does not exist";
		    					byte[] errorPacket = error.getBytes();
		    					DatagramPacket errorPack = new DatagramPacket(errorPacket, errorPacket.length, ipAddress, port);
			    				udpSocket.send(errorPack);
		    				}
		            	}
